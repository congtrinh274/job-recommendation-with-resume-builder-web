...
const JobItem = ({ job }) => {
    const { title, level, location, salary, expiredDate, approved, description } = job;

    const formattedDate = new Date(expiredDate).toLocaleDateString();

    return (
        <div
            className={`relative bg-white p-4 border shadow-md rounded-lg transition-all duration-300  ${
                approved ? 'border-b-4 border-green-500' : 'border-b-4 border-red-500'
